...
function parseStack({ callsite, renderPathRelative, originalMethod, onUs, showColors, isValidationError, }) {
    const params = {
        callsiteStr: ':',
        prevLines: '\n',
        ...
        if (exists) {
            ...
            if (!theLine || theLine.trim() === '') {
                ...
                params.prevLines =
                    '\n' +
                        highlightedLines
                            .map((l, i) => chalk.grey(renderN(i + start + 1, lineNumber + start + 1) + ' ') +
                            chalk.reset() +
                            l)
