...
import { PrismaClientInitializationError, PrismaClientKnownRequestError, PrismaClientRustPanicError, PrismaClientUnknownRequestError, } from '.';
import { Dataloader } from './Dataloader';
import { DMMFClass } from './dmmf';
import { DMMF } from './dmmf-types';
import { getLogLevel } from './getLogLevel';
import { mergeBy } from './mergeBy';
import { Args, makeDocument, transformDocument, unpack, } from './query';
    ...
    class NewPrismaClient {
            ...
            try {
                ...
                this._engineConfig = {
                    ...
                    logQueries: options.log &&
                        Boolean(typeof options.log === 'string'
                            ? options.log === 'query'
                    ...
                    for (const log of options.log) {
                        ...
                        if (level) {
                            this.$on(level, (event) => {
                                ...
                                console.error(chalk[colorMap[level]](`prisma:${level}`.padEnd(13)) +
        ...
        $executeRaw(stringOrTemplateStringsArray, ...values) {
            const doRequest = (runInTransaction = false, transactionId) => {
                try {
                    const promise = this.$executeRawInternal(runInTransaction, transactionId ?? null, stringOrTemplateStringsArray, ...values);
        ...
        $queryRaw(stringOrTemplateStringsArray, ...values) {
            const doRequest = (runInTransaction = false, transactionId) => {
                try {
                    const promise = this.$queryRawInternal(runInTransaction, transactionId ?? null, stringOrTemplateStringsArray, ...values);
  ...
  __internal: {
    engine: {
      enableEngineDebugMode: true
        ...
        _executeRequest({ args, clientMethod, dataPath, callsite, runInTransaction, action, model, headers, transactionId, }) {
            ...
            if (Debug.enabled('prisma-client')) {
                ...
                debug(`Prisma Client call:`);
                debug(`prisma.${clientMethod}(${printJsonWithErrors({
                    ast: args,
                    keyPaths: [],
                    valuePaths: [],
                    missingItems: [],
        ...
        _bootstrapClient() {
            const clients = this._dmmf.mappings.modelOperations.reduce((acc, mapping) => {
                ...
                const prismaClient = ({ operation, actionName, args, dataPath, modelName, }) => {
...
export class PrismaClientFetcher {
    ...
    async request({ document, dataPath = [], rootField, typeName, isList, callsite, clientMethod, runInTransaction, showColors, engineHook, args, headers, transactionId, }) {
...
export function getOperation(action) {
    if (action === DMMF.ModelAction.findMany ||
...
//# sourceMappingURL=getPrismaClient.js.map